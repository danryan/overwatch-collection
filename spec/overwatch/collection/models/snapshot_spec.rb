...
module Overwatch
  describe Snapshot do
    let(:data) do
      lambda { {:load_average => { :one_minute => rand(100.0), :five_minutes => rand(100.0) }} }
    end
    let(:resource) { Overwatch::Resource.create(:name => "foo") }
    
    describe "new record" do
      it "should create a new snapshot" do
        snapshot = resource.snapshots.create(:data => data.call)
    ...
    describe "#update_attribute_keys" do      
      it "should update the resource's available attribute keys" do
        resource.snapshots.create(:data => data.call)
